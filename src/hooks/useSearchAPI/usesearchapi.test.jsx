...
describe('useSearchAPI Custom Hook', () => {
  ...
  test('tests resetFilters', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(`${rootUrl}/search/?fulltext=blah&theme=dkan`);
    expect(result.current.selectedFacets).toEqual({ theme: ['dkan'] });
    ...
    expect(axios.get).toHaveBeenCalledWith(`${rootUrl}/search/?fulltext=blah&page=3&theme=dkan`);
    expect(result.current.selectedFacets).toEqual({ theme: ['dkan'] });
    expect(result.current.fulltext).toEqual('blah');
