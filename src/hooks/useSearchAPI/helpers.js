...
export function isSelectedFacet(currentFacet, selectedFacets) {
  const isInSelectedFacets = selectedFacets[currentFacet.key]
    ? selectedFacets[currentFacet.key].indexOf(currentFacet.value)
    : -1;
...
export async function fetchDatasets(rootUrl, options, additionalParams) {
  ...
  return await axios.get(
    `${rootUrl}/search/?${queryString.stringify(params, {
      arrayFormat: 'comma',
