...
const useMetastoreDataset = (datasetId, rootAPIUrl, additionalParams = {}) => {
  ...
  const additionalParamsString = Object.keys(additionalParams).length
    ? `&${qs.stringify(additionalParams)}`
    : '';
  useEffect(() => {
    async function fetchData() {
      return axios
        .get(
          `${rootUrl}/metastore/schemas/dataset/items/${id}?show-reference-ids${additionalParamsString}`
        )
